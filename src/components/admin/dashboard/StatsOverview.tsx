  ...
  return (
    <StatsGrid>
      <StatCard
        ...
        change={`+${stats.users.newThisMonth} 이번 달`}
        icon="👥"
      ...
      <StatCard
        ...
        change={`${stats.projects.total}개 중`}
        icon="⏳"
        gradient={theme.colors.gradientPurple}
      />
      <StatCard
        title="신고된 게시글"
        value={stats.posts.reported}
        change={stats.posts.hidden > 0 ? `${stats.posts.hidden}개 숨김` : '처리 완료'}
        icon="🚨"
      ...
      <StatCard
        ...
        change={`${Math.round((stats.users.active / stats.users.total) * 100)}% 활성`}
        icon="✅"
